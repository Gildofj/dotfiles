...
function M.status(icon, status)
  ...
  return {
    ...
    color = function()
      return { fg = Utils.color(colors[status()] or colors.ok) }
    end,
  }
end
...
function M.cmp_source(name, icon)
  icon = icon or Core.icons.kinds[name:sub(1, 1):upper() .. name:sub(2)]
  local started = false
